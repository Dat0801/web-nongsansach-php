...
class Product extends Controller
{
    ...
    {
        $validateTenHang = 'required|min:5|max:30|unique:HangHoa:TenHang';

        if ($id != null) {
            $validateTenHang = 'required|min:5|max:30|unique:HangHoa:TenHang:MaHang=' . $id . '';
        }

        $request->rules([
            'TenHang' => $validateTenHang,
            'DVT' => 'required',
    ...
    public function editProduct()
    {
        ...
        if ($request->isPost()) {
            ...
            $validate = $this->validateProduct($request, $id);
